...
sql_table_name = 'param_erpe'

class ObjParamERPEWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        uic.loadUi('widgets\\erpe_page\\changeParamsERPE.ui', self)
        ...
        self.sqlArhive = get_all_params_erpe(para_names_in_sql)

        # add gaps from sql
        self.table_gap = QtWidgets.QTableWidget()
        self.table_gap.resize(590, 380)
        self.table_gap.setColumnCount(1)
        self.table_gap.setHorizontalHeaderLabels(['Перфорация, мм'])
        self.table_gap.setColumnWidth(0, 200)

        self.table_gap.setRowCount(len(self.sqlArhive['Перфорация, мм']) + 10)

        add_items_into_table(table_name=self.table_gap,
                             values_array=self.sqlArhive['Перфорация, мм'])
